...
from dataset.Dataset_grant import GRANTDataset
from typing import Optional
...
class Config(object):
    ...
    def __init__(self, conf_file_path=None, seed=None, exp_name=None, log=True):
        ...
        dataset_path = r"D:\PythonProject\chatbot\dataset\preprocessed\grant\train.csv"
        dataset = GRANTDataset(dataset_path)
        ...
        try:
            ...
            category_counts = [
                ...
                max(7, int(self.df["weekday"].nunique())),  # 计算 `is_holiday`
                max(12,int(self.df["month"].nunique())),
                max(2, int(self.df["is_weekend"].nunique())),  # 计算 `day_of_week`
...
if __name__ == '__main__':
    ...
    cnf = Config(conf_file_path = r"D:\PythonProject\chatbot\config\config\Grant.yaml",
                 exp_name="Grant")
