...
class Config(object):
    ...
    def __init__(self, conf_file_path=None, seed=None, exp_name=None, log=True):
        ...
        dataset_path = r"D:\PythonProject\chatbot\dataset\preprocessed\CESNET\train.csv"
        dataset = CESNETDataset(dataset_path)
        ...
        try:
            ...
            category_counts = [
                ...
                max(2, int(self.df["is_weekend"].nunique())),  # 计算 `is_holiday`
                max(2,int(self.df["is_holiday"].nunique())),
                max(7, int(self.df["day_of_week"].nunique())),  # 计算 `day_of_week`
...
if __name__ == '__main__':
    ...
    cnf = Config(conf_file_path = r"D:\PythonProject\chatbot\config\config\CESNET.yaml",
                 exp_name="CESNET")
